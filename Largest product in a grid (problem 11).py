...
for j in range(17):
    for i in range(17):
        ...
        diagonal.append(prod)
for j in range(17):
    for i in range(17):
            prod = grid[j][i+3]*grid[j+1][i+2]*grid[j+2][i+1]*grid[j+3][i]
            diagonal.append(prod)
highest.append(max(horizontal))
highest.append(max(vertical))
highest.append(max(diagonal))
answer = max(highest)
print(answer)
